...
export class BlockMenuView implements ReturnType<NonNullable<SuggestionOptions['render']>> {
  ...
  public onStart(props: SuggestionProps) {
    ...
    this._element.classList.add('dip-prose-mirror-bm')
  ...
  public onKeyDown(props: SuggestionKeyDownProps) {
    ...
    if (props.event.key === 'ArrowUp') {
      ...
      this._element.classList.add('keyboard-navigating')
    ...
    if (this._items.length) {
      ...
      for (let i = 0; i < this._items.length; i++) {
        ...
        if (item === '|') {
          ...
          root.classList.add('dip-prose-mirror-bm-divider')
          this._nodes.push(root)
        } else {
          const root = document.createElement('button')
          root.name = item.id
          root.classList.add('dip-prose-mirror-bm-button')
          // icon
          if (item.icon) {
            const icon = document.createElement('div')
            icon.classList.add('dip-prose-mirror-bm-button-icon')
            icon.innerHTML = item.icon
            root.append(icon)
          }
          // name
          const name = document.createElement('div')
          name.classList.add('dip-prose-mirror-bm-button-name')
          name.textContent = item.name
          root.append(name)
          // shortcut
          if (item.shortcut) {
            const shortcut = document.createElement('div')
            shortcut.classList.add('dip-prose-mirror-bm-button-shortcut')
          ...
          root.addEventListener('mouseover', () => {
            if (this._element && this._index !== i && !this._keyboardNavigating) {
              this._select(i)
            }
          })
          root.addEventListener('mousemove', () => {
            // 鼠标移动时重置键盘导航标志
            if (this._keyboardNavigating) {
              this._keyboardNavigating = false
              if (this._element) {
                this._element.classList.remove('keyboard-navigating')
              }
            }
          })
          this._nodes.push(root)
      ...
      view.classList.add('dip-prose-mirror-bm-empty')
