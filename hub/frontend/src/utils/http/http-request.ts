...
const createHttpRequest = curry(
  (method: string, url: string, options: OptionsType | undefined) => {
    ...
    const axiosConfig = {
      ...
      headers: {
        ...
        ...(body instanceof FormData
          ? {}
          : { 'Content-Type': 'application/json;charset=UTF-8' }), // formData无需设置 content-type，axios 会自动设置
        ...headers,
      },
      timeout,
...
const cacheableHttpFn = () => {
  ...
  return (
    method: string,
    url: string,
    options?: OptionsType & { expires?: number }
  ) => {
