...
在项目根目录或 `src` 目录下创建 `public-path.js` 和生命周期文件。

#### 1.2.1 创建 `public-path.js`
...
- 通过 `window.__POWERED_BY_QIANKUN__` 判断是否在 qiankun 环境中运行
...
- 检查浏览器控制台错误信息
- 确认生命周期函数已正确导出
- 检查 `window.__POWERED_BY_QIANKUN__` 是否为 `true`
